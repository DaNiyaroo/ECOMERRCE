...
async function updateAddress(req, res, next) {
  try {
    const { region, referencePoint, street, house, room, userID } = req.body
    const id = req.params.id

    if (userID) {
      const [[user]] = await pool.query("SELECT * FROM address WHERE ID = ?", userID);
      if (!user) {
    ...
    const updateAddress = {
      ...
      userID: user ? userID : address[0].userID
